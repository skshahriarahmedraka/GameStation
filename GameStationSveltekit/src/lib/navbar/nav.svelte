...
<div
	class="flex h-[52px] w-full flex-row items-center justify-center bg-gray-900 sm:gap-2 md:gap-4 lg:gap-4 xl:gap-4 xs:gap-1 xs:justify-around sm:justify-around  "
>
	{#if !ShowSearchBar}
		<button
			on:click={() => {
		...
		<button
			...
			class="  font-Poppins text-base text-slate-300  hover:text-white xs:text-xs sm:text-sm"
		>
			Browse
		</button>
		<button
	...
	<!-- search bar  -->
	<div class=" flex justify-center  font-Poppins text-gray-200">
	...
	<div class="sm:hidden md:hidden lg:hidden xl:hidden xs:hidden xxl:contents">
		<List
			on:click={() => {}}
	...
	<div
		class=" flex h-[60%] w-fit flex-row items-center gap-2 rounded-2xl border-0 border-blue-500 bg-sky-300 bg-opacity-30 pr-2  sm:hidden md:hidden lg:hidden xl:hidden xs:hidden xxl:contents"
	>
	...
	<div class=" xl:contents xxl:hidden  xs:h-6 xs:w-6 ">
