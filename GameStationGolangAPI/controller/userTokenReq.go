...
func (H *DatabaseCollections) ProfileTokenReq() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if count > 0 {
			...
			err = H.Mongo.Collection(os.Getenv("ADMIN_MONEY_MANAGE_COL")).FindOneAndUpdate(ctx, filter, update, opts).Decode(&UpdatedDocument)
			if err != nil {
